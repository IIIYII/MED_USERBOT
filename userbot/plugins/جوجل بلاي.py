...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Medthon"
...
async def apkr(event):
    ...
    try:
        ...
        app_details += "\n\n<b>حــ۫͜ـســﻭ الفخامه: </b> <a href='https://t.me/L_U_2'>لــ الاستفسـار</a>"
        app_details += "\n\n===> 𝗠𝙚𝙙𝗧𝗵𝗼𝗻 - @MedThon ® <==="
        await event.edit(app_details, link_preview=True, parse_mode="HTML")
    except IndexError:
...
async def Med(event):
    ...
    tap = await bot.inline_query(zelzal, Medr)
...
CMD_HELP.update(
    {
        ...
        \n**الشـرح ⦂ **لـ تحميـل التطبيقـات والالعـاب غير محدود وبأحجـام كبيـره تصـل الى 2 جيجـا بايت .. قـم بكتابـة الاسـم بالانجلـش بطـريقـه صحيحـه والا فلن يستطيـع البـوت التعـرف ع طلبـك او قم اولاً باستخـدام الامر `.متجر + اسم التطبيق` بالعربي للحصول ع اسم التطبيق بالانجلـش .. حصـرياً فقـط ع سـورس مدثــون ™✓ \
        \n\n**╮•❐ الامـر ⦂ **`.فلم + اسـم الفلـم`\
        \n**الشـرح ⦂ **لـ تحميـل الافـلام الاجنبيـه بدقـه عاليـه قم بكتابـة الاسـم بالانجلـش بطـريقـه صحيحـه والا فلن يستطيـع البـوت التعـرف ع طلبـك .. حصـرياً فقـط ع سـورس مدثــون ™✓ \
