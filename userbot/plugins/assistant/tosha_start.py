    ...
    if LOAD_MYBOT == "False":
        ...
        if BOT_PIC:
            await tgbot.send_message(event.chat_id, BOT_PIC,
                                  ...
                                  buttons=[
                                      [Button.inline(
                                          "⚜️ 𝗠𝗘𝗗𝗧𝗵𝗼𝗻 - 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 ⁦⚜️", data="Ralls")]
                                  ])
        else:
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        buttons=[
            ...
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/MedThon")]
        ])
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط ريس ثـون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/MedThon/Ralls_USERBOT")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://t.me/MedThon/105")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/MedThon")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @MedThon", alert=True)
