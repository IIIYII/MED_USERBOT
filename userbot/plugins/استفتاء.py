...
async def pollcreator(Medpoll):
    reply_to_id = None
    if Medpoll.reply_to_msg_id:
        reply_to_id = Medpoll.reply_to_msg_id
    string = "".join(Medpoll.text.split(maxsplit=1)[1:])
    if not string:
        ...
        try:
            ...
            await edit_or_reply(Medpoll, "⌔∮ هذه الدردشة تحظر استطلاعات الرأي. ")
        except exception as e:
            await edit_or_reply(Medpoll, str(e))
    else:
        Medinput = string.split("|")
        if len(Medinput) > 2 and len(Medinput) < 12:
            options = Build_Poll(Medinput[1:])
            try:
                await bot.send_message(
                    Medpoll.chat_id,
                    file=InputMediaPoll(
