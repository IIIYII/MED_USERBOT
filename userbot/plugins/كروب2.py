...
async def _(event):
    ...
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if i.bot:
            ...
            n += 1
    if input_str:
        required_string = """𓆰 𝑺𝑶𝑼𝑹𝑪𝑬 𝗠𝗘𝗗𝗧𝗵𝗼𝗻  - 𝑮𝑹𝑼𝑶𝑷 𝑫𝑨𝑻𝑨 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻
...
async def get_users(event):
    ...
    MedTHON = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
    ...
    async for user in event.client.iter_participants(MedTHON.full_chat.id):
        try:
...
async def _(event):
    ...
    Medthonevent = await edit_or_reply(event, "**❈╎جـاري اتمـام العمليـه .. الرجـاء الانتظـار 🚸**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await Medthonevent.edit(f"**❈╎تـم  تفلـيش {total} عضـو .. بنجـاح ☑️🗑**")
    
async def ban_user(chat_id, i, rights):
    try:
        await Medthon(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)
